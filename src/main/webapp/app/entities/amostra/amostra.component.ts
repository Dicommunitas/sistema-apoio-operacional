...
export class AmostraComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.amostraService
        ...
        .subscribe(
          (res: HttpResponse<IAmostra[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInAmostras(): void {
    this.eventSubscriber = this.eventManager.subscribe('amostraListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAmostra[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
