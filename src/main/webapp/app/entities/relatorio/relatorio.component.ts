...
export class RelatorioComponent implements OnInit, OnDestroy {
  ...
  loadAll(): void {
    if (this.currentSearch) {
      this.relatorioService
        ...
        .subscribe((res: HttpResponse<IRelatorio[]>) => this.paginateRelatorios(res.body, res.headers));
      return;
    }
  ...
  registerChangeInRelatorios(): void {
    this.eventSubscriber = this.eventManager.subscribe('relatorioListModification', () => this.reset());
  }
  ...
  protected paginateRelatorios(data: IRelatorio[] | null, headers: HttpHeaders): void {
    const headersLink = headers.get('link');
    this.links = this.parseLinks.parse(headersLink ? headersLink : '');
    if (data) {
      for (let i = 0; i < data.length; i++) {
        this.relatorios.push(data[i]);
      }
    }
  }
}
