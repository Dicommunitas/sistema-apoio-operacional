...
export class ProblemaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.problemaService
        ...
        .subscribe(
          (res: HttpResponse<IProblema[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInProblemas(): void {
    this.eventSubscriber = this.eventManager.subscribe('problemaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IProblema[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
