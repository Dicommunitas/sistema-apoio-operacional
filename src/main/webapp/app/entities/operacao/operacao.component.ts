...
export class OperacaoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.operacaoService
        ...
        .subscribe(
          (res: HttpResponse<IOperacao[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInOperacaos(): void {
    this.eventSubscriber = this.eventManager.subscribe('operacaoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IOperacao[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
